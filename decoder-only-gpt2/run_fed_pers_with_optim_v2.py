...
from models.modeling_gpt2 import (
    GPT2ForPromptTuning,
    GPT2ForDiscriminativeClassification,
)
...
def train(args, train_dataset, model, tokenizer, logger):
    ...
    # global_c and client_c in scaffold
    global_c = {}
        ...
        for client_idx in tqdm(selected_clients):
            ...
            for _ in range(args.local_eps):
                for batch_idx, batch in enumerate(train_dataloader_list[client_idx]):
                    ...
                    if args.method_type == 'generative':
...
def main():
    ...
    model = GPT2ForPromptTuning.from_pretrained(
            args.model_name_or_path,
            config=config,
        )
