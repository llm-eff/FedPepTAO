...
class PromptGeneratorWithNaivePromptGenerator(nn.Module):
    ...
    def forward(self, hidden_states=None, seq_lengths=None):
        sequence_output = hidden_states[torch.arange(hidden_states.size(0)), seq_lengths, :]
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        if self.add_prompt_layer > 0:
            prompt_attention_mask = self.get_extended_attention_mask(prompt_attention_mask)
        if prompt_embedding is not None and self.add_prompt_layer < 1:
        ...
        output_shape = (batch_size, input_ids.size(1), hidden_states.size(-1))
        ...
        for i, (block, layer_past) in enumerate(zip(self.h, past_key_values)):
            ...
            # attention mask, adopted from ptuningv2
            if past_key_values is not None:
                if i in self.config.prompt_layer_list:
                    attention_mask[:, :, :, :self.config.num_prompt_tokens] = -0
                else:
                    attention_mask[:, :, :, :self.config.num_prompt_tokens] = -10000
            else:
                raise NotImplementedError()
...
class GPT2ForDiscriminativeClassification(GPT2ForSequenceClassification):
...
class GPT2ForPromptTuning(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        prefix_attention_mask = torch.ones(input_ids.shape[0], self.pre_seq_len).to(self.transformer.device)
        ...
        if labels is not None:
            ...
            # shift_logits = lm_logits[..., self.num_prompt_tokens:-1, :].contiguous()
