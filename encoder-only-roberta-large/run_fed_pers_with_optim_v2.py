...
from evaluations import aggregate_plus, evaluate, personalized_evaluate, predict, aggregate, quiet_evaluate, ratio_minus, update_client_c, average_weights_plus
...
def train(args, train_dataset, model, tokenizer, logger):
    ...
    # partition training set (and test set if to do personalization)
    ...
    # global c and client c in scaffold
    global_c = {}
    ...
    for epoch in train_iterator:
        ...
        generator_param_list = [] # use to aggregate clients in this epoch
        weight_list = []        
        local_steps = [] # num steps for each client
        ...
        for client_idx in tqdm(selected_clients):
            ...
            # personalization: a portion of local parameters will update through 
            # training, but will not participate in aggregation
            ...
            if args.server_optimizer == 'adam':
                if not pre_grad:
                    ...
                    for client_idx, idx in enumerate(selected_clients):
                        ...
                        # rebuttal: get the percentage of client ci / mi
                        c_norm = torch.norm(clients_c[idx]['embedding.weight'].data)
                        m_norm = torch.norm(server_optimizer.v['prefix_encoder.embedding.weight'].data)
                        ratio = c_norm / m_norm
                    ...
                    print(f"Ratio list: {str(ratio_list)}")
                    print(f"Mean ratio: {np.mean(ratio_list)}")
                    ratio_list_all.append(np.mean(ratio_list))
    ...
    print(f"overall ratio mean: {np.mean(ratio_list_all)}")
...
def main():
    ...
    logger.info(f"Tuning layers: {args.prompt_layer_list}")
    ...
    if args.do_train:
        ...
        global_step, tr_loss, best_all_metric = train(args, train_dataset, model, tokenizer, logger)
